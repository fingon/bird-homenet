             ...
             || (ifap->pa_priority == highest_link_pa_priority && ifap->pa_pxlen == highest_link_pa_pxlen && en->lsa.rt > po->router_id)))
      {
        PARSE_LSA_AC_ASP_START(asp, ifap)
        {
          ...
          if(net_in_net(addr, len, self_r_px->px.addr, self_r_px->px.len) || net_in_net(self_r_px->px.addr, self_r_px->px.len, addr, len))
          {
            ...
            configure_ifa_del_prefix(self_r_px);
            deassigned_prefix = 1;
            change = 1;
            break;
          }
                  ...
                  unsigned int *change, struct prefix_node *self_r_px)
{
  ...
  switch(choose_prefix(&pxu, &px, *used, po->router_id, ifa))
  {
    case PXCHOOSE_SUCCESS:
      ...
      *change = 1;
      *pxchoose_success = 1;
      configure_ifa_add_prefix(px.addr, px.len, po->router_id, po->router_id, ifa->pa_priority, ifa);
      break;
                    ...
                    unsigned int *pxchoose_success, unsigned int *change, struct prefix_node *self_r_px)
{
  ...
  if(can_assign)
  {
    ...
    *change = 1;
    *pxchoose_success = 1;
    configure_ifa_add_prefix(*spec_addr, *spec_len, po->router_id, po->router_id, ifa->pa_priority, ifa);
  }
}
...
ospf_pxassign_remove_iface(struct ospf_iface *ifa)
{
  ...
  struct proto_ospf *po = oa->po;
  struct proto *p = &po->proto;
  struct ospf_iface_prefixes *ip;

  if (EMPTY_LIST(ifa->asp_list))
    return;
  if (!ifa->iface)
    {
      WALK_LIST_FIRST(asp, ifa->asp_list)
        {
          OSPF_TRACE(D_EVENTS, "Interface %s: removing prefix %I/%d", ifa->iface->name, asp->px.addr, asp->px.len);
          configure_ifa_del_prefix(asp);
  ...
  /* Store the entries for reuse. */
  ip = mb_allocz(p->pool, sizeof(struct ospf_iface_prefixes));
  ip->iface = ifa->iface;
  init_list(&ip->asp_list);
  WALK_LIST_DELSAFE(asp, aspn, ifa->asp_list)
    {
      rem_node(NODE asp);
      add_tail(&ip->asp_list, NODE asp);
    }

  ip->valid = 10;
  add_tail(&po->ip_list, NODE ip);
  OSPF_TRACE(D_EVENTS, "Storing prefix list for interface %s[p]",
             ifa->iface->name,
             ifa->iface);
}

void
ospf_pxassign_new_iface(struct ospf_iface *ifa)
{
  struct ospf_area *oa = ifa->oa;
  struct proto_ospf *po = oa->po;
  struct proto *p = &po->proto;
  ...
  /* First off, try to find the stored interface to reuse. */
  init_list(&ifa->asp_list);
  WALK_LIST(ip, po->ip_list)
    {
      if (ip->iface == ifa->iface)
        {
          OSPF_TRACE(D_EVENTS, "Reusing old usp/asp list for interface %s[%p]",
                     ifa->iface->name,
                     ifa->iface);
          /* Copy nodes from ip->list to ifa->asp_list */
          WALK_LIST_DELSAFE(asp, aspn, ip->asp_list)
            {
              rem_node(NODE asp);
              add_tail(&ifa->asp_list, NODE asp);
              asp->valid = 10;
              /* FIXME: Won't be culled in the next two runs of prefix
               * assignment algorithm. Rather arbitrary logic, I'm
               * afraid. */
            }
          /* Schedule AC LSA */
          schedule_ac_lsa(oa);
  ...
  OSPF_TRACE(D_EVENTS, "No old information for interface %s[%p]",
             ifa->iface->name, ifa->iface);
}
